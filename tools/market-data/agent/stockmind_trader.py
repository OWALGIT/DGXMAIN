...
Each cycle: read the datalake -> market snapshot -> feed the AI brain (local
freellmapi gateway) the snapshot + its OWN recent decisions and realized P&L so
it reflects and adapts -> rebalance a virtual book with costs & risk caps ->
log EVERYTHING to journal.jsonl (that log is the learning record). No real broker.
...
MAX_GROSS = CFG.get('max_gross', 1.5); MAX_NAME = CFG.get('max_name', 0.25); CAPITAL = CFG.get('capital', 10000)
...
def rebalance(st, targets, prices):
    ...
    for t in list(st['positions']) + list(tw):
        ...
        px = prices[t]; cur = st['positions'].get(t, {}).get('qty', 0.0)
        tgt = tw.get(t, 0.0)*eq/px; dq = tgt - cur
        if abs(dq*px) < eq*0.01: continue                      # 1% dust band — curb churn/costs
...
def ask_brain(snap, ctx, st, prices, memory):
    ...
    sysmsg = ("You are a disciplined systematic PM running a LONG/SHORT paper book, and you LEARN from your own track record. "
              "You receive a market snapshot, your current weights, and your recent decisions with their realized P&L. "
              "Reflect on what worked, then decide TARGET WEIGHTS for the next hour. "
              f"Rules: long positive, short negative; |per-name|<={MAX_NAME}; sum|weights|<={MAX_GROSS} (rest cash). "
              "Prefer names above their 150d MA; de-risk when breadth weak, VIX high, or HYG below its 150d MA. "
              "Avoid needless churn — every trade costs 5bp, so only move weights when your view actually changes. "
    ...
    for m in [MODEL] + [x for x in CFG.get('fallback_models', ['minimax-m3','gemini-3.5-flash','glm-4.7']) if x != MODEL]:
        body = json.dumps({'model':m,'temperature':0.3,'max_tokens':800,'stream':False,
...
def rule_fallback(snap, ctx):
    ...
    tw = {}
    if picks:
        top = sorted(picks, key=picks.get, reverse=True)[:8]; budget = 0.5 if risk_off else 0.9
    ...
    return {'targets':tw,'thesis':f"rule-based ({'risk-off' if risk_off else 'risk-on'} trend-follow)",
            'reflection':'(deterministic rule — no learning)','risk':'fallback brain'}

def cycle():
    st = load_state(); snap, prices, ctx = snapshot()
    if not prices: log('no prices — datalake unreachable'); return
    hist = journal_tail(8)
    memory = [{'ts': r.get('ts'), 'ret_pct': r.get('ret_total_pct'), 'cycle_pnl': r.get('cycle_pnl'),
               'brain': r.get('brain'), 'thesis': (r.get('thesis') or '')[:100]} for r in hist]
    prev_eq = hist[-1]['equity'] if hist else st.get('equity0', CAPITAL)
    dec, brain = ask_brain(snap, ctx, st, prices, memory)
    if dec is None:
        log(f"AI brain unavailable ({brain}); rule fallback")
    ...
    rec = {'ts': now(), 'asof': ctx.get('asof'), 'brain': brain, 'equity': round(eq1, 2),
           'cycle_pnl': round(eq1-prev_eq, 2), 'ret_total_pct': round((eq1/st['equity0']-1)*100, 2),
    ...
    log(f"[SIM] cycle {st['cycles']} | brain={brain} | equity=${eq1:,.0f} ({rec['ret_total_pct']:+.2f}%) "
        f"| pnl ${rec['cycle_pnl']:+.2f} | {len(st['positions'])} pos | cash {rec['cash_pct']}% | fills {len(fills)}")
